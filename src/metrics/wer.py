...
class ArgmaxWERMetric(BaseMetric):
    def __init__(self, text_encoder, use_beam_search=False, beam_size=3, *args, **kwargs):
        """
        Calculate Word Error Metric with the choosen decoding method.
        Args:
            use_beam_search (bool): use beam_search (if text_encoder is initialized with LM, premade version with LM is used)
    ...
    def __call__(
        ...
        for prediction, log_probs_matrix, length, target_text in zip(predictions, log_probs, lengths, text):
